        ...
        <div class="card-body pt-4 p-3">
            <form action="{{ route('series.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="col-md-6">
                        <div class="form-group has-validation">
                            ...
                            <input class="form-control @error('name') border border-danger rounded-3 @enderror"
                                type="text" placeholder="Nama Series" name="name" id="name"
                    ...
                    <div class="col-md-6">
                        <div class="form-group has-validation">
                            ...
                            <input class="form-control @error('slug') border border-danger rounded-3 @enderror"
                                type="text" placeholder="slug-series" name="slug" value="{{ old('slug') }}">
                            @error('slug')
                                <p class="text-danger text-xs mt-2">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="form-group has-validation">
                            ...
                            <select name="next_series_id" id="next_series_id"
                                ...
                                @foreach ($seriesOptions as $s)
                                    <option value="{{ $s->id }}"
                                        {{ (string) old('next_series_id') === (string) $s->id ? 'selected' : '' }}>
                                        {{ $s->name }}</option>
                                @endforeach
                            </select>
                    ...
                    <div class="col-md-6">
                        <div class="form-group has-validation">
                            ...
                            <select name="previous_series_id" id="previous_series_id"
                                ...
                                @foreach ($seriesOptions as $s)
                                    <option value="{{ $s->id }}"
                                        {{ (string) old('previous_series_id') === (string) $s->id ? 'selected' : '' }}>
                                        {{ $s->name }}</option>
                                @endforeach
                            </select>
                    ...
                    <div class="col-md-6">
                        <div class="form-group has-validation">
                            ...
                            <select name="genres[]" id="genres"
                                ...
                                @foreach ($genres as $g)
                                    <option value="{{ $g->id }}"
                                        {{ collect(old('genres', []))->contains($g->id) ? 'selected' : '' }}>
                                        {{ $g->name }}</option>
                                @endforeach
                            </select>
                            @error('genres')
                    ...
                    <div class="col-md-6">
                        <div class="form-group has-validation">
                            ...
                            <input class="form-control @error('studios') border border-danger rounded-3 @enderror"
                                type="text" placeholder="Studios" name="studios" value="{{ old('studios') }}">
                            @error('studios')
                                <p class="text-danger text-xs mt-2">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div class="col-md-6">
                        <div class="form-group has-validation">
                            ...
                            <input class="form-control @error('folder_id') border border-danger rounded-3 @enderror"
                                type="text" placeholder="Folder ID" name="folder_id" value="{{ old('folder_id') }}">
                            @error('folder_id')
                                <p class="text-danger text-xs mt-2">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div class="col-12">
                        <div class="form-group has-validation">
                            ...
                            <textarea class="form-control @error('synopsis') border border-danger rounded-3 @enderror" name="synopsis"
                                rows="3" placeholder="Sinopsis">{{ old('synopsis') }}</textarea>
                            @error('synopsis')
                                <p class="text-danger text-xs mt-2">{{ $message }}</p>
                            @enderror
                        </div>
                ...
                <div class="d-flex justify-content-end">
                    <button type="button" class="btn btn-primary btn-md mt-4 mb-4 me-2" data-toggle="modal"
                        data-target="#exampleModal">{{ 'Get Data From MyAnimeList' }}</button>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="modal-search-bar">
                        <input type="text" class="form-control" id="searchDonghua"
                            placeholder="Type to Search Donghua/Anime">
                    </div>
                    ...
                    $.ajax({
                        ...
                        success: (res) => {
                            ...
                            res.data.forEach(item => {
                                let year = item.aired && item.aired.from ?
                            ...
                            $('.select-anime').on('click', function() {
                                let data = $(this).data();
                                const malGenres = Array.isArray(data.genres) ?
                                ...
                                const missing = malGenres.filter(name => !
                                    availableNames.some(opt => opt.name ===
                                        name));
                                ...
                                if (missing.length) {
                                    genresError
                                        .removeClass('d-none')
                                        .text(
                                            `Genre berikut belum tersedia di sistem: ${missing.map(m => m.charAt(0).toUpperCase() + m.slice(1)).join(', ')}`
                                            );
                                } else {
                                ...
                                $('input[name="name"]').val(data.title).trigger(
                                    'input');
                                $('input[name="total_episodes"]').val(data
                                    .episodes);
                                $('input[name="rating"]').val(Number.parseFloat(
                                    data.rating ?? 0).toFixed(1));
                                $('textarea[name="synopsis"]').val(data
                                    .synopsis);
                                ...
                                if (data.date) {
                                    ...
                                    const month = String(releaseDate
                                    .getMonth() + 1).padStart(2, '0');
                                    const day = String(releaseDate.getDate())
                                        .padStart(2, '0');
                            ...
                            `);
                                ...
                                // Tutup modal
                                $('#exampleModal button[data-dismiss="modal"]')
                                    .trigger('click');
                            });
                        },
                        error: (err) => {
